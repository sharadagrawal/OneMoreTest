    ...
    (function (Rides) {
        var PushNotificationsService = (function () {
            ...
            PushNotificationsService.prototype.tokenHandler = function (result) {
                if (this.mobileClient) {
                    var hub = new NotificationHub(this.mobileClient);
                    var template = "{" + "\"aps\" : {" + "\"alert\":\"Request received\"" + "}," + "\"employeeId\":\"$(employeeId)\"," + "\"latitude\":\"$(latitude)\"," + "\"longitude\":\"$(longitude)\"" + "}";
                    hub.apns.register(result, this.tagsToRegister, 'myTemplate', template, null).done(function () {
                        console.log('Registered with hub.');
                    }).fail(function (error) {
                        console.log('Failed registering with hub: ' + error);
                    });
                ...
                switch (e.event) {
                    case 'registered':
                        if (e.regid.length > 0) {
                            ...
                            if (this.mobileClient) {
                                var hub = new NotificationHub(this.mobileClient);
                                var template = "{ \"data\" : {" + "\"title\":\"Request received\"," + "\"message\":\"Touch to see the information\"," + "\"employeeId\":\"$(employeeId)\"," + "\"latitude\":\"$(latitude)\"," + "\"longitude\":\"$(longitude)\"" + "}}";
...
describe('SettingsController Tests', function () {
